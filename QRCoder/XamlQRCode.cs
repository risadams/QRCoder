...
namespace QRCoder
{
    public class XamlQRCode : AbstractQRCode, IDisposable
    {
        ...
        public DrawingImage GetGraphic(int pixelsPerModule) => GetGraphic(pixelsPerModule, true);
        ...
        public DrawingImage GetGraphic(Size viewBox, bool drawQuietZones = true) => GetGraphic(viewBox, new SolidColorBrush(Colors.Black), new SolidColorBrush(Colors.White), drawQuietZones);
        ...
        public DrawingImage GetGraphic(Size viewBox, Brush darkBrush, Brush lightBrush, bool drawQuietZones = true)
        {
            ...
            for (var xi = offsetModules; xi < drawableModulesCount + offsetModules; xi++)
            {
                ...
                {
                    if (QrCodeData.ModuleMatrix[yi][xi]) @group.Children.Add(new RectangleGeometry(new Rect(x, y, unitsPerModule, unitsPerModule)));
                    y += unitsPerModule;
                }
        ...
        private int GetDrawableModulesCount(bool drawQuietZones = true) => QrCodeData.ModuleMatrix.Count - (drawQuietZones ? 0 : 8);
