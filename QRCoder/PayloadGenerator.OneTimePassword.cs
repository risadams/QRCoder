...
namespace QRCoder
{
    public static partial class PayloadGenerator
    {
        public class OneTimePassword : Payload
        {
            ...
            {
                if (Period == null) throw new Exception("Period must be set when using OneTimePasswordAuthType.TOTP");
            ...
            {
                if (String40Methods.IsNullOrWhiteSpace(Secret)) throw new Exception("Secret must be a filled out base32 encoded string");
                ...
                {
                    if (Issuer.Contains(":")) throw new Exception("Issuer must not have a ':'");
                    escapedIssuer = Uri.EscapeUriString(Issuer);
                }

                if (!String40Methods.IsNullOrWhiteSpace(Label))
                {
                    if (Label.Contains(":")) throw new Exception("Label must not have a ':'");
                    escapedLabel = Uri.EscapeUriString(Label);
                }

                if (escapedLabel != null)
                {
                    if (escapedIssuer != null) escapedLabel = escapedIssuer + ":" + escapedLabel;
                }
                ...
                if (escapedLabel != null) sb.Append(escapedLabel);

                sb.Append("?secret=" + strippedSecret);

                if (escapedIssuer != null) sb.Append("&issuer=" + escapedIssuer);
