    ...
    public sealed class PngByteQRCode : AbstractQRCode, IDisposable
    {
        ...
        private byte[] DrawScanlines(int pixelsPerModule)
        {
            ...
            for (var y = 0; y < matrixSize; y++)
            {
                ...
                for (var x = 0; x < matrixSize; x++)
                {
                    ...
                    for (; pixelIndex < endIndex; pixelIndex++) scanlines[scanlineOffset + 1 + pixelIndex / 8] |= (byte) (0x80 >> (pixelIndex % 8));
                }

                // Copy the scanline required number of times.
                for (var copyCount = 1; copyCount < pixelsPerModule; copyCount++) Array.Copy(scanlines, scanlineOffset, scanlines, scanlineOffset + copyCount * bytesPerScanline, bytesPerScanline);
            }
        ...
        private sealed class PngBuilder : IDisposable
        {
            ...
            private static readonly uint[] CrcTable =
            {
                ...
                0x62DD1DDF, 0x15DA2D49, 0x8CD37CF3, 0xFBD44C65, 0x4DB26158, 0x3AB551CE, 0xA3BC0074, 0xD4BB30E2, 0x4ADFA541, 0x3DD895D7, 0xA4D1C46D, 0xD3D6F4FB, 0x4369E96A, 0x346ED9FC, 0xAD678846, 0xDA60B8D0, 0x44042D73, 0x33031DE5, 0xAA0A4C5F, 0xDD0D7CC9, 0x5005713C, 0x270241AA, 0xBE0B1010, 0xC90C2086, 0x5768B525, 0x206F85B3, 0xB966D409, 0xCE61E49F,
                0x5EDEF90E, 0x29D9C998, 0xB0D09822, 0xC7D7A8B4, 0x59B33D17, 0x2EB40D81, 0xB7BD5C3B, 0xC0BA6CAD,
                0xEDB88320, 0x9ABFB3B6, 0x03B6E20C, 0x74B1D29A, 0xEAD54739, 0x9DD277AF, 0x04DB2615, 0x73DC1683, 0xE3630B12, 0x94643B84, 0x0D6D6A3E, 0x7A6A5AA8, 0xE40ECF0B, 0x9309FF9D, 0x0A00AE27, 0x7D079EB1, 0xF00F9344, 0x8708A3D2, 0x1E01F268, 0x6906C2FE, 0xF762575D, 0x806567CB, 0x196C3671, 0x6E6B06E7, 0xFED41B76, 0x89D32BE0, 0x10DA7A5A, 0x67DD4ACC,
            ...
            public void WritePalette(params byte[][] rgbaColors)
            {
                ...
                foreach (var color in rgbaColors) stream.WriteByte(color.Length > Alpha ? color[Alpha] : Opaque);
                WriteChunkEnd();
            }
            ...
            {
                using (var deflateStream = new DeflateStream(output, CompressionMode.Compress, true)) deflateStream.Write(bytes, 0, bytes.Length);
            }
            ...
            private static uint Crc32(byte[] data, int index, int length)
            {
                ...
                var end                             = index + length;
                for (var n = index; n < end; n++) c = CrcTable[(c ^ data[n]) & 0xff] ^ (c >> 8);
