...
namespace QRCoder
{
    public class QRCodeData : IDisposable
    {
        ...
        public QRCodeData(byte[] rawData, Compression compressMode)
        {
            ...
            if (compressMode == Compression.Deflate)
                using (var input = new MemoryStream(bytes.ToArray()))
                {
                    ...
                    {
                        using (var dstream = new DeflateStream(input, CompressionMode.Decompress)) Stream4Methods.CopyTo(dstream, output);
                ...
                using (var input = new MemoryStream(bytes.ToArray()))
                {
                    using (var output = new MemoryStream())
                    {
                        using (var dstream = new GZipStream(input, CompressionMode.Decompress)) Stream4Methods.CopyTo(dstream, output);
                        bytes = new List<byte>(output.ToArray());
                    }
                }
            ...
            foreach (var b in bytes)
            {
                ...
                for (var i = 7; i >= 0; i--) modules.Enqueue((b & (1 << i)) != 0);
            }
            ...
            for (var y = 0; y < sideLen; y++)
            {
                ...
                for (var x = 0; x < sideLen; x++) ModuleMatrix[y][x] = modules.Dequeue();
            }
        }
        ...
        public byte[] GetRawData(Compression compressMode)
        {
            ...
            for (var i = 0; i < 8 - ModuleMatrix.Count * ModuleMatrix.Count % 8; i++) dataQueue.Enqueue(0);

            //Process queue
            while (dataQueue.Count > 0)
            {
                byte b                         = 0;
                for (var i = 7; i >= 0; i--) b += (byte) (dataQueue.Dequeue() << i);
                bytes.Add(b);
            }
            ...
            if (compressMode == Compression.Deflate)
                using (var output = new MemoryStream())
                {
                    using (var dstream = new DeflateStream(output, CompressionMode.Compress)) dstream.Write(rawData, 0, rawData.Length);
                    rawData = output.ToArray();
                }
            else if (compressMode == Compression.GZip)
                using (var output = new MemoryStream())
                {
                    using (var gzipStream = new GZipStream(output, CompressionMode.Compress, true)) gzipStream.Write(rawData, 0, rawData.Length);
                    rawData = output.ToArray();
                }
